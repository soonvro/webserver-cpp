...
/*   Updated: 2023/10/31 14:21:58 by insub            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  if (argc == 5) {
    ...
    */
    ...
    if (!pid) {
      ...
      close(pipe_fd[1]);
      command[1] = 0;
      char* cmd[3] = {"/opt/homebrew/bin/php-cgi", argv[3], NULL};
      if (!strcmp(command[0], "php-cgi"))
        execve("/opt/homebrew/bin/php-cgi", NULL, environ);
      // personal mac -> /opt/homebrew/bin/php-cgi
      // cluster mac  -> 1) install php on host
      //              -> 2) /Users/$(USER)/.brew/bin/php-cgi
      else if (!strcmp(command[0], "cgi_tester"))
        execve("./cgi_tester", NULL, environ);
      // exit(1);
    } else {
      // close(pipe_fd[1]);
      int nbytes;
