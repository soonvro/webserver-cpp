...
void doTest(const char* test_case) {
  using namespace std;
  HttpDecoder hd;
  unsigned int n_read = hd.execute(test_case, static_cast<unsigned int>(std::strlen(test_case)));
  if (static_cast<unsigned int>(std::strlen(test_case)) != n_read) {
    cout << "error: n_read is not correct. n_read: " << n_read <<
            ", Faliure Flag: " << ((hd._errno == HPS::kFailure) ? "Failure" : "Success") << endl;
    return;
  }
  cout << "Success" << endl;
  ...
  cout << " flag: " << endl;
  cout << "  FlagChunked            : " << ((hd._flag & HPS::kFlagChunked) ? 1 : 0) << endl;
  ...
  cout << "  FlagTrailing           : " << ((hd._flag & HPS::kFlagTrailing) ? 1 : 0) << endl;
  cout << "  FlagSkipbody           : " << ((hd._flag & HPS::kFlagSkipbody) ? 1 : 0) << endl;
  cout << "  FlagContentlength      : " << ((hd._flag & HPS::kFlagContentlength) ? 1 : 0) << endl;
}
